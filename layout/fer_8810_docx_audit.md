...
| C2 | **Heat sink** | Condenser circ-water → **Black Sea once-through** (TGB condenser end toward sea) (§8.10.2); CCWS → seawater (§8.8.9) | Mech-draft **cooling tower CTW** `[ASSUMED]` (Z6) — and the **team PFD (`aegis40_pfd.drawio.png`) still shows CT-1 + CWP** | Sinop coastal makes once-through natural (better η, kills CTW footprint + plume). **But the PFD is the process SSOT and shows a tower.** Decide once, then fix PFD *or* layout. EPZ/dose §8.1 and intake structure follow. |
| C3 | **TES technology** | **Thermochemical** zeolite 13X sorption, ~390 t, charge ~280 °C/15 bar pass-out steam (§8.10.3, §8.10.2) | **Two-tank sensible Therminol-66** 265/268 °C (PFD: TK-C/TK-H, IHX E-2, discharge boiler E-3) | Direct contradiction with the PFD. Alisher's scope — he must pick one; FER §8.9 + §8.10 + PFD must say the same thing. |
...
| C5 | **Containment concept** | RPV submerged in a **steel-lined water pool** as UHS, NuScale-style; "pool as ultimate heat sink for **>100 h**" (§8.10.4) | **Dry steel-lined containment Ø15 m + IRWST annulus** (viewer, CAD, rxb drawings); 72 h grace (`operator_grace_period`) | Architecture-level divergence: pool-type vs dry containment changes containment pressure analysis, IRWST role, RXB section drawings, CAD diorama. **Decide explicitly** — if pool-type wins, my §8.6/§8.5 text + all RXB visuals need rework. |
