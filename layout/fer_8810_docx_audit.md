...
|---|---|---|---|---|
| C1 | **Island scheme** — ✅ **RESOLVED 2026-06-26: compact in-line spine** | docx "two islands / one steam corridor" | ~~three islands~~ → **compact spine adopted** | **RESOLVED toward the merged single-steam-corridor scheme** — a single in-line spine SFB→RXB→TB→TCES→SOE (the docx's E-CI logic), aux/electrical flanking. Construction economy; all safety separations preserved (seismic gap, **H₂ bulk storage offset to SE corner ~123 m**, fire, security tiers). zones.md Z9; detailed in `planning/COMPACT_LAYOUT_PROPOSAL.md`. **Graphical recut (site plan/elevation/CAD/viewer) = next phase (P1).** |
...
| C5 | **Containment concept** — 🟢 **NARROWED 2026-06-26 (team): internal IRWST pool, CAREM-style** | RPV coupled to an **in-containment water pool** as the passive heat sink (CAREM-25 precedent) | was "dry Ø15 m + IRWST annulus" vs NuScale submerged-pool — **now an internal-IRWST-pool concept** | **Decision: internal IRWST pool inside containment** (CAREM-style). Resolves the IRWST role: the pool IS the passive safety UHS, gravity-fed DHRS/ECCS condense back into it (no external penetration). **Still to confirm:** suppression-pool vs dry-condensing containment pressure response (gates `containment_pt_analysis`); RXB sections + CAD diorama recut to show the internal pool. By-similarity safety basis: CAREM-25 (NRC/ARN-reviewed iPWR). |
