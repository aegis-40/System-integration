...
| **Operating principle** | Refueling machine over the reactor pool transfers assemblies through a **flooded transfer canal** (water = shield + coolant) to the SFP fuel-handling machine. Core is soluble-boron-free (decision #3), so the SFP credits **no soluble boron** — subcriticality is held by **fixed neutron-absorber rack geometry + burnup credit**, with acceptance k_eff(95/95) ≤ 0.95 in **unborated** water at the maximum-reactivity assembly (Cabrera 2023 method, replicated in Samira's `openmc_bridge.build_storage_rack_model`). SFP holds ≥ full-core-offload + multi-cycle capacity. Dry-cask staging for long-term. |
