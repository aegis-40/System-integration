...
| **Operating principle** | Refueling machine over the reactor pool transfers assemblies through a **flooded transfer canal** (water = shield + coolant) to the SFP fuel-handling machine. SFP holds assemblies in a borated `[note: boron-free core decision #3 — SFP may still use fixed neutron-absorber racks instead of soluble boron; confirm rack type]` storage rack at ≥ full-core-offload + multi-cycle capacity. Dry-cask staging for long-term. |
