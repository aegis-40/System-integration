...
| **EBIS** | (ATWS branch only) Diverse shutdown system #2 | Subcriticality without rods | Emergency Boron Injection System (passive, DAS-armed) — see `trip_signals.md` E6 | DAS (`ebis_actuation`) |
...
flowchart LR
  ...
  RT -->|fail = ATWS| EBIS{"Diverse shutdown?<br/>EBIS boron inj."}
  ...
  EBIS -->|success| EFW2{"EFW / PRHR<br/>available?"}
  EBIS -->|fail| CD2["CD-2<br/>ATWS core damage"]
  ...
  class RT,EFW,PRHR,IRWST,EBIS,EFW2 top;
```
...
| S5 | ¬RT·EBIS·EFW2 | OK-4 | Safe (diverse shutdown) | ~1e-6 | EBIS boron injection + cooling |
| S6 | ¬RT·EBIS·¬EFW2 | **CD-3** | Core damage | ~1e-9 | ATWS + cooling failure |
| S7 | ¬RT·¬EBIS | **CD-2** | Core damage | ~1e-10 | ATWS unmitigated |
...
5. **ATWS (¬RT):** DAS detects sustained high flux + temperature with no rod-insertion confirmation → actuates **EBIS** (Emergency Boron Injection System), the diverse second shutdown system (§ `trip_signals.md` §6, E6). Subcriticality achieved by liquid poison, sharing no failure mode with the rods.
...
| # | Item | Impact | Owner |
|---|---|---|---|
| 1 | EBIS boron mass/concentration sizing (§6.10 standalone cold-subcritical) | Confirms EBIS branch success criterion | Samira (OpenMC borated-core case) |
